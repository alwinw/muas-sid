...
% linearise about the mean values
% NB: This means that the model regressors are actually perturbations
% Xb = Xb - ...
%     [mean(resampled_data.airspeed);
%      mean(resampled_data.angle_of_attack);
%      mean(resampled_data.sideslip);
%      mean(resampled_data.angular_velocity_x);
%      mean(resampled_data.angular_velocity_y);
%      mean(resampled_data.angular_velocity_z);
%      mean(resampled_data.aileron);
%      mean(resampled_data.elevator);
%      mean(resampled_data.rudder);
%      mean(resampled_data.angle_of_attack_rate);
%      mean(resampled_data.sideslip_rate)]';
...
for i=1:size(z,2)
    ...
    ms = ModelStructure(z(:,i), Kb, Xb, 'Order', 0, 'Confidence', [0.99, 0.95]);
...
%%
% t = log_data.imu.time - log_data.imu.time(1);
% q = log_data.imu.gyro_y;
% 
% figure(2); clf;
% subplot(211);
% idx = t >= 494.7 & t <= 510.0;
...
% subplot(311);
% [q,t] = resample(q, t, Fs);
% plot_frequency_distribution(t, q);
% 
% subplot(312);
% q = bandstop(q, [0.5 14.5], Fs);
% q = lowpass(q, 15.5, Fs);
% plot_frequency_distribution(t, q);
% 
% subplot(313);
% idx = t >= 494.7 & t <= 510;
% q = smooth(t, q, 10, 'loess');
% plot_frequency_distribution(t, q);
% 
% figure(2);
% subplot(212);
...
%%
%{
model_CL = @(V) 3.09 - 0.177*V + 0.00278*V.^2;
model_CD = @(alpha, delta_e) 0.13 + 0.457*alpha + 0.959*alpha.^2 + ...
                             2.34*delta_e + 1.71*delta_e.^2;

figure(1); clf;

legStr = {};
i = 1;
for alpha=0.00:0.01:0.05
    
    legStr{i} = sprintf('$\\alpha=%.2f^o$', alpha*180/pi);
    V = 21:0.1:25;

    subplot(211);
    hold on;
    plot(model_CL(V), model_CD(V,alpha), '-o');
    grid on;
    xlabel('C_L');
    ylabel('C_D');
    ...
    subplot(212);
    hold on;
    L_D = model_CL(V) ./ model_CD(V,alpha);
    plot(V, L_D, '-o');
    grid on;
    xlabel('V');
    ylabel('C_L/C_D');
